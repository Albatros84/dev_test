...
class PanelProvider < ActiveRecord::Base
  TimeNews = 'http://time.com'.freeze
  AjaxGoogleApi = 'https://ajax.googleapis.com/ajax/services/feed/find?v=1.0&q=news'.freeze

  def self.panel1_price(letter = 'a')
    Nokogiri::HTML(open(TimeNews)).text.scan(letter.to_s).count / 100.0
  end
